...
define([
    ...
    "dojo/html",
    "dojo/_base/event",
    "dojo/text!CustomCheckbox/widget/templates/CustomCheckbox.html"
    ...
    return declare("CustomCheckbox.widget.CustomCheckbox", [_WidgetBase, _TemplatedMixin], {
        ...
        _loadData: function () {
            ...
            if (this.inputNode) {
				...
				//Start evaluation if our checkbox is editable
                if ( this.editable &&
                    !this.readOnly &&
                    !this._contextObj.isReadonlyAttr(this.dataAttr)
                ) {
					if( this.condEditableAttr != "" && this.condEditableAttr != null ) {
						var currentValue = this._contextObj.get(this.condEditableAttr);
                        if( currentValue != null )
                            currentValue = currentValue.toString().toLowerCase();
                        for( var i = 0; i < this.conditions.length ; i ++ ) {
                            if( this.conditions[i].condition.toLowerCase() == currentValue ) {
                                showEditable = true;
                                break;
                            }
                        }
        ...
        _resetSubscriptions: function () {
            ...
            if (this._contextObj) {
                ...
                var condAttrHandle = null;
                if( this.condEditableAttr != null ) {
                     condAttrHandle= this.subscribe({
                        guid: this._contextObj.getGuid(),
                        attr: this.condEditableAttr,
                        callback: dojoLang.hitch(this, function (guid, attr, attrValue) {
                            this._updateRendering();
                        })
                    });
...
require(["CustomCheckbox/widget/CustomCheckbox"], function () {
    "use strict";
});